...
using UnityEngine;
/// <summary>
/// To load/save meshes into the hard drive
/// </summary>
public class MeshSerializer
{
    ...
    public static byte[] WriteMesh(Mesh mesh, bool saveTangents)
    {
        ...
        for(int i = 0; i < mesh.subMeshCount; i++)
        {
            ...
            //Debug.Log("submeshTriangles " + submeshTriangles[i].Length);
        }
        ...
        //Debug.Log("W" + submeshCount);
        for (int i = 0; i < submeshCount; i++)
        {
            var subTriLength = (ushort)submeshTriangles[i].Length;
            //Debug.Log("W\t" + subTriLength);
            buf.Write(subTriLength);
        }
        //buf.Write(triCount);
        buf.Write(format);
        // vertex components
    ...
    public static Mesh ReadMesh(byte[] bytes)
    {
        ...
        int[] subMeshTriLengths = new int[submeshCount];
        for (i = 0; i < submeshCount; i++)
        {
            subMeshTriLengths[i] = buf.ReadUInt16();
        }
        ...
        // triangle submeshes count
        mesh.subMeshCount = submeshCount;
        for (i = 0; i < submeshCount; i++)
        {
            // triangle submeshes indices
            int[] tris = new int[subMeshTriLengths[i]];
            for (int j = 0; j < tris.Length; j++)
            {
                tris[j] = buf.ReadUInt16();
            }
